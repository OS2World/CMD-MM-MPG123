...
char *find_next_file (int argc, char *argv[], int previous)
{
    static FILE *listfile = NULL;
    static char line[1024];
    static unsigned long linenumber = 1;
...
void init_input (int argc, char *argv[])
{
    ...
    while ((tempstr = find_next_file(argc, argv,0))) {
...
char *get_next_file(int argc, char **argv, int previous)
{
...
/* samuel */
    ...
    if (!shuffle) {
   return find_next_file(argc, argv,previous);
    }
...
/* samuel */
extern char *boostprio;
extern char *normalprio;
extern int lockdevice;
extern int numbuffers;
extern int trashbuffers;
extern unsigned long volume;

topt opts[] = {

    /* samuel */
    {'B', "boostprio",   GLO_ARG | GLO_CHAR, 0, &boostprio,    0},
    {'N', "normalprio",  GLO_ARG | GLO_CHAR, 0, &normalprio,   0},
    {'l', "lockdevice",  0,                  0, &lockdevice,   TRUE},
    {'b', "numbuffers",  GLO_ARG | GLO_NUM,  0, &numbuffers,   0},
    {'V', "volume",      GLO_ARG | GLO_NUM,  0, &volume,       0},
    {'T', "trash",       0,                  0, &trashbuffers, FALSE},
    {'w', "wavdir",    GLO_ARG | GLO_CHAR, 0, &wavdir,     0},
...
/* samuel */
extern int playingframe;
extern int rew,ffwd,jumptoframe,previous;
...
int main(int argc, char *argv[])
{
   ...
   int sfd,secs,totalsecs;
    ...
    if(wavdir) outmode = DECODE_WAVFILE;
...
#ifdef SHUFFLESUPPORT
   while ((fname = get_next_file(argc, argv, previous))) {
#else
   while ((fname = find_next_file(argc, argv, previous))) {
#endif
      ...
      if(outmode==DECODE_WAVFILE)
      {
         char drive[_MAX_DRIVE], dir[_MAX_DIR];
         char fname2[_MAX_FNAME], ext[_MAX_EXT];
         char temp[_MAX_PATH + _MAX_FNAME];
         char *slash;

         _splitpath (fname, drive, dir, fname2, ext);

         strcpy(temp,wavdir);
         slash = strchr(temp,'\0')-1;
         if(*slash++ != '/')
         {
            *slash++ = '/';
            *slash++ = '\0';
		...
		init = 1;
      for(fr.frameNum=0;read_frame(&fr) && numframes && !intflag;fr.frameNum++) {
         ...
         if(jumptoframe >= 0)
         {
            int rewffwdframes = jumptoframe - fr.frameNum;

            if(rewffwdframes > 0)
               fr.frameNum += forward_frame(&fr,rewffwdframes);
            else
               fr.frameNum -= back_frame(&fr,-rewffwdframes);

            jumptoframe = -1;
         }

         if(rew)
            fr.frameNum -= back_frame(&fr,7);

         if(ffwd)
            fr.frameNum += forward_frame(&fr,6);

         /* This formula seems to work at least for
          * MPEG 1.0/2.0 layer 3 streams.
          */
         sfd = freqs[fr.sampling_frequency] * (fr.lsf + 1);
         if(outmode == DECODE_AUDIO)
            secs = (playingframe * (fr.lay==1 ? 384 : 1152) + sfd / 2) / sfd;
         else
            secs = (fr.frameNum * (fr.lay==1 ? 384 : 1152) + sfd / 2) / sfd;
         totalsecs = size_stream()/(tabsel_123[fr.lsf][fr.lay-1][fr.bitrate_index] * 125);
         if(totalsecs)
         {
            fprintf(stderr,"[%d:%02d]  Remaining: [%d:%02d]  Total: [%d:%02d]\r",
                     secs / 60, secs % 60,
                     (totalsecs - secs) / 60, (totalsecs - secs) % 60,
                     totalsecs / 60, totalsecs % 60);
         }
         else
            fprintf(stderr,"[%d:%02d]  Remaining: unknown\r", secs / 60, secs % 60);

			if(verbose) {
            if (verbose > 1 || !(fr.frameNum & 0xf))
               fprintf(stderr, "\r{%4d} ",fr.frameNum);
      ...
      if(intflag) audio_trash_buffers(&ai);
      close_stream();
...
static void print_title(void)
{
    ...
    fprintf(stderr,"Modified and ported to OS/2 by Samuel Audet <guardia@cam.org> (C) 1998 v0.20\n");
}
...
static void usage(char *dummy)  /* print syntax & exit */
{
   ...
   fprintf(stderr,"   -b n  number of 16kB buffers [32]    -f n  change scalefactor [32768]\n");
   fprintf(stderr,"   -r n  override samplerate [auto]     -w d  output WAV file to dir d\n");

   /* samuel */
   fprintf(stderr,"   -a d  set audio device               -l    Lock Audio Device\n");
   number = audio_get_devices(NULL,0);
   for(i=1;i<=number;i++)
   {
      audio_get_devices(buffer,i);
      fprintf(stderr,"Device ID #%d = %s\n",i,buffer);
   }
   fprintf(stderr,"   -B p  boost priority [300]           -N p  normal priority [231] \n");
   fprintf(stderr,"   -8    8 bit output                   -T    do not trash buffers on seek \n");
   ...
   fprintf(stderr,"   -u a  HTTP authentication string     -V n  volume [100]\n");
